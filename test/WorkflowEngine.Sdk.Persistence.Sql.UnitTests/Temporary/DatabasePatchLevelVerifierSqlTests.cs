...
namespace WorkflowEngine.Sdk.Persistence.Sql.IntegrationTests.Temporary
{
    public class DatabasePatchLevelVerifierSqlTests : AbstractDatabaseTest
    {
        ...
        {
            // Act & Assert
            await Assert.ThrowsAsync<FulcrumBusinessRuleException>(async () => await _patchLevelVerifier.VerifyDatabasePatchLevel(1));
        }
        ...
        public void Patches_Database_To_Sdk_Level_When_Greater_Than_Database_Level()
        {
            ...
            lock (RollbackLock)
            {
                ...
                // Act
                ThreadHelper.CallAsyncFromSync(async () => await _patchLevelVerifierForDrop.VerifyDatabasePatchLevel(DatabasePatchSettings.DatabasePatchVersion));
                var patchLevel = connection.QuerySingle<long>("SELECT MAX(Version) FROM DbVersion");

                // Assert
                Assert.Equal(DatabasePatchSettings.DatabasePatchVersion, patchLevel);
            }
        }
        ...
        public async Task Does_Nothing_When_Sdk_Level_Is_Equal_To_Database_Level()
        {
            ...
            await _patchLevelVerifier.VerifyDatabasePatchLevel(DatabasePatchSettings.DatabasePatchVersion);
